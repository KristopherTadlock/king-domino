...
def _advance_to_learner(env, opponent, learner_player: int, max_steps: int = 128) -> None:
    steps = 0
...
def run_ppo_smoke(
    *,
    ...
    while collected_samples < max(0, steps):
        ...
        _advance_to_learner(env, opponent, learner_player)
        ...
        _obs, reward, done, info = step_legal(env, action, observe=False)
        if "error" in info:
            illegal_actions += 1
