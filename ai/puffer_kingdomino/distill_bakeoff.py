...
def _policy_path(output_dir: Path, dataset: Path, head: str, samples: int, seed: int) -> Path:
    return output_dir / f"distilled_{dataset.stem}_{head}_{samples}_{seed}.pt"
...
def run_bakeoff(
    *,
    ...
    pairs = games // 2
    for head in heads:
        policy_path = _policy_path(output_dir, dataset, head, samples, seed)
        if force_train or not policy_path.exists():
            print(f"Training {head} head -> {policy_path}", file=sys.stderr, flush=True)
