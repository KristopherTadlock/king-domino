"""Evaluate a prototype factorized action-head checkpoint."""
...
@torch.no_grad()
def _choose_factorized_action(model, env, action_parts, legal_buffer, obs_buffer) -> int:
    ...
    for game_index in range(games):
        ...
        while not env.done:
            if env.current_player == 0:
                action = _choose_factorized_action(model, env, action_parts, legal_buffer, obs_buffer)
            else:
